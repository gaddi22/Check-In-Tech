...
public class EventManager extends javax.swing.JFrame{
    ...
    private void initComponents() {
        ...
        yearField.setToolTipText("Enter 2-digit day.");
        eventPrevPanel.add(yearField, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 90, 50, -1));
        ...
        colonLabel.setText(":");
        eventPrevPanel.add(colonLabel, new org.netbeans.lib.awtextra.AbsoluteConstraints(194, 160, -1, -1));
        eventPrevPanel.add(endMinField, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 160, 30, -1));

        minDurationLabel.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        minDurationLabel.setText("Duration:");
        eventPrevPanel.add(minDurationLabel, new org.netbeans.lib.awtextra.AbsoluteConstraints(29, 198, -1, -1));

        minHrDurationField.setToolTipText("Enter minimum time required to be counted as present.");
        eventPrevPanel.add(minHrDurationField, new org.netbeans.lib.awtextra.AbsoluteConstraints(101, 196, 30, -1));

        minHrLabel.setFont(minHrLabel.getFont());
        minHrLabel.setText("Hours");
        eventPrevPanel.add(minHrLabel, new org.netbeans.lib.awtextra.AbsoluteConstraints(135, 199, 40, -1));

        minMinLabel.setFont(minMinLabel.getFont());
        minMinLabel.setText("Minutes");
        eventPrevPanel.add(minMinLabel, new org.netbeans.lib.awtextra.AbsoluteConstraints(210, 199, 50, -1));

        minMinDurationField.setToolTipText("Enter minimum time required to be counted as present.");
        eventPrevPanel.add(minMinDurationField, new org.netbeans.lib.awtextra.AbsoluteConstraints(175, 196, 30, -1));
    ...
    private void addUpdateBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addUpdateBtnActionPerformed
        boolean validInput = true;  boolean dateCk = true;
        boolean eventNameInput = true; 
        boolean correctTime = true; 
        boolean durCk = true;
        
        // Verify an event name is specified.
        if(eventNameField.getText().equals("")){eventNameInput = false;}
                
        //try{
            // Verify inputs are integers.
            /*Integer.parseInt(yearField.getText()); int month = Integer.parseInt(monthField.getText()); int day = Integer.parseInt(dayField.getText()); // Verify date.
            int startHr = Integer.parseInt(startHrField.getText()); int startMin = Integer.parseInt(startMinField.getText()); // Verify start time.
            int endHr = Integer.parseInt(endHrField.getText()); int endMin = Integer.parseInt(endMinField.getText()); // Verify end time.
            int minHr = Integer.parseInt(minHrDurationField.getText()); int minMin = Integer.parseInt(minMinDurationField.getText());
            
            // Calculate total time and duration.
            int ttlTime = (endHr*60+endMin) - (startHr*60+ startMin);
            int ttlDurr = minHr*60+minMin;
            
            // Verify times.
            if(!(startHr > -1 && startHr < 25 && endHr > -1 && endHr < 25)){correctTime = false;}
            if(!(startMin > -1 && startMin < 61 && endMin > -1 && endMin < 61)){correctTime = false;}
            
            // Verify duration doesn't exceed total time.
            if(ttlDurr > ttlTime) {durCk = false;}
        ...
        if(validInput && eventNameInput && dateCk && correctTime && durCk){
            // javax.swing.JOptionPane.showConfirmDialog(null, "Confirm change?" );
            if(eventManagerTable.getSelectedRow() == -1){
                String startTime = yearField.getText() + "-" +monthField.getText() +"-" +dayField.getText() +" "
                    +startHrField.getText() +":" +startMinField.getText() +":00";
                String endTime = yearField.getText() + "-" +monthField.getText() +"-" +dayField.getText() +" "
                    +endHrField.getText() +":" +endMinField.getText() +":00";
                String durTime = minHrDurationField.getText() +":" + minMinDurationField.getText() + ":00";
                DBConnector.createEvent(startTime, durTime, eventNameField.getText(), endTime, LoginScreen.getUsername());
            } else {
                int index = eventManagerTable.getSelectedRow();
                String eventID = events[index][0];
                String startTime = yearField.getText() + "-" +monthField.getText() +"-" +dayField.getText() +" "
                    +startHrField.getText() +":" +startMinField.getText() +":00";
                String endTime = yearField.getText() + "-" +monthField.getText() +"-" +dayField.getText() +" "
                    +endHrField.getText() +":" +endMinField.getText() +":00";
                String durTime = minHrDurationField.getText() +":" + minMinDurationField.getText() +":00";
                DBConnector.updateEventInfo(eventID, startTime, durTime, eventNameField.getText(), endTime, LoginScreen.getUsername());
            }
        } /*else if (!eventNameInput){ javax.swing.JOptionPane.showMessageDialog(null, "Event name must be specified.");}
        
        else if (!dateCk) {javax.swing.JOptionPane.showMessageDialog(null, "Please verify date.");}
        
        else if (!correctTime) {javax.swing.JOptionPane.showMessageDialog(null, "Hours must be formatted between 0 and 24. Minutes must be between 0 and 60.");} 
       
        else if (!durCk) {javax.swing.JOptionPane.showMessageDialog(null, "The minimum time cannot be greater than the total time.");} 
            
        else {javax.swing.JOptionPane.showMessageDialog(null, "All dates and times must be numerical.");} */
        setEvents();
        new EventManager().setVisible(true);
        this.setVisible(false);
    ...
    private void clearBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_clearBtnActionPerformed
        eventNameField.setText(""); monthField.setText(""); startHrField.setText(""); minHrDurationField.setText(""); startMinField.setText("");
        yearField.setText(""); dayField.setText(""); endHrField.setText(""); endMinField.getText();minMinDurationField.setText(""); endMinField.setText("");
    ...
    private void deleteBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteBtnActionPerformed
        ...
        int index = eventManagerTable.getSelectedRow();
        DBConnector.deleteEvent(events[index][0].toString());
        setEvents();
        new EventManager().setVisible(true);
        this.setVisible(false);
    ...
    private void rosterBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rosterBtnActionPerformed
        ...
        if (!(eventNameField.getText().equals(""))){
            new EventRosterManager(events[index][0]).setVisible(true);
            this.setVisible(false);
        }
        else
    ...
    private void eventManagerTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_eventManagerTableMouseClicked
        ...
        int index = eventManagerTable.getSelectedRow();
        Object [] selectedRow = new Object [5];

        selectedRow[0] = model.getValueAt(index, 0);
        selectedRow[1] = model.getValueAt(index, 1);
        selectedRow[2] = model.getValueAt(index, 2);
        selectedRow[3] = model.getValueAt(index, 3);
        selectedRow[4] = events[index][2];
        
        String year = selectedRow[1].toString().substring(0, 4);
        String month = selectedRow[1].toString().substring(5, 7);
        String day = selectedRow[1].toString().substring(8, 10);
        String startHrTime = selectedRow[2].toString().substring(0,3);
        String startMinTime = selectedRow[2].toString().substring(4,6);
        String endHrTime = selectedRow[3].toString().substring(0,3);
        String endMinTime = selectedRow[3].toString().substring(4,6);
        String durHr = selectedRow[4].toString().substring(0,2);
        String durMin = selectedRow[4].toString().substring(3,5);

        // Set all fields.
        eventNameField.setText(selectedRow[0].toString());
    ...
    private static void loadEventTable(){
        ...
        try {
            ...
            for(int i=0; i < eventTable.length; i++){
                eventTable[i][0] = events[i][3];
                eventTable[i][1] = events[i][1].substring(0,10);
                eventTable[i][2] = events[i][1].substring(10);
                eventTable[i][3] = events[i][4].substring(10);
            }
